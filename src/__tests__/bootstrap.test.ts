...
describe("assembleRelay", () => {
  it("assembles a working relay from config with the default backends", () => {
    ...
    const reg = relay.register({ handle: "h", did: "did:key:zR", agentCard: { name: "a", url: "u", version: "1", protocolVersion: "0.3.0", did: "did:key:zR" } })
    expect(reg.ok).toBe(true)
    expect(relay.agentCard().did).toBe(config.did)
  })

  it("honors injected overrides (clock, logger, tokens, backends)", () => {
    ...
    expect(registry.getByHandle("h")?.registeredAt).toBe(123)
  })
})
