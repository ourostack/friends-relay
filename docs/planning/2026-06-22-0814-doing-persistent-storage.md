...
- [ ] RF4: the bound is enforced exactly + atomically (the atomic enqueue rejects at the cap, count AND bytes); parity with the memory store's behavior + reasons preserved.
- [ ] Full gate green: build + typecheck + lint clean, 100%/100%/100%/100% coverage, all tests (incl. dual-backend parity + restart + the interop content-blind headline) pass.
- [ ] Content-blind assertion still passes against Postgres (the inbox row shape is unchanged by RF4; RF3 touches only credential/invite at-rest form, never the inbox blob).
