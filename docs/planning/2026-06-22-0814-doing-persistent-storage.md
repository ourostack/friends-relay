...
- [ ] RF3: the stored credential columns + invite token column hold the SHA-256 hash, not the plaintext; a valid credential/token still authenticates, an invalid one rejects; the plaintext is unrecoverable from any stored row; restart + parity tests stay green.
