...
## Completion Criteria
- [x] Every state family from the audit persists through the Postgres backend, or is explicitly documented as intentionally ephemeral (rate-limiter buckets — with rationale). *(registrations, inbox queue, invites, credentials → Postgres adapters; rate-limiter buckets stay in-memory, documented in README + a NOTE block.)*
- [x] In-memory backend remains the default; with no `RELAY_STORE`/`DATABASE_URL` set, behavior + the existing hermetic suite are unchanged (modulo the mechanical sync→async signature shift, which is internal). *(`RELAY_STORE` absent ⇒ memory; the original 157 hermetic tests still pass, converted to `await`.)*
- [x] Postgres backend selected purely by env/config; misconfiguration fails loud at startup (consistent with `loadConfig`'s fail-loud contract). *(`resolveStore` throws on postgres-without-DATABASE_URL + unrecognized RELAY_STORE; config.test.ts covers both.)*
- [x] Content-blind invariant preserved AND proven against Postgres: the durable inbox persists only `{ recipientDid, opaque A2AMessage blob (jsonb), queueId, enqueuedAt, expiresAt, sizeBytes }` — no plaintext, no key, no decoded `sealed.ct`, never indexed on ciphertext. *(Pinned: inbox does NOT store recipient_did — DID match is pre-store in Relay.enqueue. pg-inbox.test.ts asserts the raw row round-trips to the opaque shape with no plaintext + no ct/recipient_did column; the parity suite asserts the interface-level content-blind shape over Postgres too.)*
- [x] Ordered-queue + ack/delete + per-handle count/byte quota + TTL semantics of `InboxStore` reproduced exactly by the Postgres adapter (the shared `store.test.ts` behavioral contract passes against BOTH backends). *(62 parity tests, 31 per backend, all green.)*
- [x] Re-registration credential rotation + invite single-use/capped consumption survive a **simulated restart** (new store instances over the same `pg-mem` db reload state). *(restart.test.ts — 3 tests: full state survives; consumed single-use invite stays rejected; rotated-away credential stays rejected.)*
- [x] 100% coverage on all new code (no `v8 ignore` on new logic except the established `bin.ts` process-wiring pattern). *(100%/100%/100%/100% — zero new `v8 ignore`; the default pool factory is covered hermetically via lazy-connect construct+end.)*
- [x] All tests pass; `npm run typecheck` + `npm run lint` clean; no warnings. *(266 tests pass; typecheck + build + lint clean.)*
- [x] README "Deploy" section + Dockerfile note updated with the new env var(s) (`RELAY_STORE`, `DATABASE_URL`) and the one-line infra-to-provision note. *(README "Durable storage (Postgres)" subsection + env table rows + Azure flexible-server B1ms infra note + content-blind-extends-to-db note; Dockerfile pure-JS-pg comment.)*
...
- 2026-06-22 09:00 Units 1–8 complete (each: tests→red→impl→green→coverage, committed + pushed per top-level unit). U1 async-seam refactor (signature-only, suite stayed green). U2 PgInboxStore + content-blind proof. U3 PgRegistryStore (recency getByDid). U4 Invite/CredentialStore seams (managers → logic-over-store). U5 Pg invite/credential adapters (spaced-arithmetic gotcha applied). U6 RELAY_STORE/DATABASE_URL config + assemblePostgresStores/buildPostgresStores + bin wiring. U7 dual-backend parity suite (62 tests, both backends, content-blind over Postgres). U8 simulated-restart durability proof (3 tests).
- 2026-06-22 09:11 Unit 9 complete (README Durable-storage section + env table + Azure B1ms infra note + content-blind-extends-to-db note; Dockerfile pure-JS-pg comment). **ALL 24 SUB-UNITS DONE.** Final full gate: 266 tests pass (14 files); coverage 100%/100%/100%/100% (462 stmts / 336 branches / 103 funcs / 433 lines); typecheck + build + lint clean; ZERO new `v8 ignore`. All 9 Completion Criteria satisfied. Status → done. (Artifact: `final-gate-coverage.txt`, `scratch-schema-validate.mjs`.)
...
## Review fixes (content-blind security review pass, folds into this branch)

**Status: all 4 fixes (RF1–RF4) complete.** Final gate green — 278 tests pass (16 files); 100%/100%/100%/100% coverage; build+typecheck+lint clean. Content-blindness intact (re-confirmed against Postgres + the interop headline). Branch `feat/persistent-storage`, 12 review-fix commits on top of `12d806a` (8 fix/docs pairs + the addendum). NOT pushed (the personal-account push credential isn't active on this machine — see note in the progress log) and NO PR opened, per scope.

A content-blind security review (content-blindness itself intact — unchanged) found 4 real issues. Each is fixed under strict TDD, same gates (100% on all 4 metrics, typecheck, lint, hermetic + dual-backend-parity + restart tests green). No PR, no Azure/deploy.

### Fixes
- **RF1 (HIGH) — http.ts unhandled rejection + hung socket.** `createServer`'s per-request `void (async()=>{…})()` had no `.catch()`; now `handle()` is async + can reject (a Postgres query throwing mid-request) → socket hangs + unhandled rejection. Fix: wrap so any `handle()` rejection writes `500` + ends the socket, logging a STATIC event name only (no connection string / message body / error detail). `createServer` gains an optional `logger` param (defaults `silentLogger`); `bin.ts` passes one.
- **RF2 (HIGH) — no `pool.on('error')` → crash on idle-conn failure.** `defaultPoolFactory` attached no error listener; `pg-pool` emits `'error'` on idle-client backend failures → unhandled → process crash on a DB failover/restart/blip. Fix: `pool.on('error', …)` logging a STATIC event (`pg_pool_error`) only, NEVER the connection string / error detail. `defaultPoolFactory` + `assemblePostgresStores` gain an optional `logger`.
- **RF3 (MEDIUM) — bearer secrets stored plaintext at rest.** `credentials.inbox_auth` + `credentials.send_credential` + invite `token` persisted in clear → a DB/backup/replica leak yields usable bearer tokens. Fix at the **store boundary, BOTH backends** (keeps parity valid): SHA-256-hash these high-entropy secrets before storing (fast hash is correct — random high-entropy, not low-entropy passwords, so no slow KDF); on lookup hash the presented value + compare (every access path is exact-match equality, so transparent). Registry `did`/`agent_card`/`key_agreement_pubkey` are PUBLIC routing data — left as-is.
- **RF4 (MEDIUM) — concurrent-enqueue quota race.** `PgInboxStore.enqueue` ran prune → count/sum → insert as 3 non-atomic autocommit statements → two concurrent posts to one handle could both pass the bound + overshoot. Fix: make the bound-enforcing enqueue **atomic** (count AND byte-sum both enforced atomically), preserving the same `quota_count`/`quota_bytes` reasons + count-first precedence.

### RF3 design decision (Design B — hash at the store boundary, `getCurrent` returns the stored hash)
The credential store has ONE column per secret serving BOTH the current-pair storage AND the reverse lookups, so plaintext cannot remain anywhere ("not recoverable from any stored row"). Therefore `getCurrent` returns the **stored hashes** (its only consumer is `CredentialManager.rotate`/`revoke`, which feeds the result straight to `deleteFor` — never a client). Rule, applied identically in BOTH backends so parity holds:
- `setCurrent` hashes plaintext → stores hash.
- `handleForInboxAuth` / `handleForSendCredential` hash the presented plaintext → query.
- `getCurrent` returns the stored hashes.
- `deleteFor` matches columns to its input **as-given** (stored-form; production feeds it `getCurrent`'s output). It does NOT re-hash — double-hashing would break revoke (deregister) + rotation-revoke.
- Invites: `setRemaining`/`getRemaining`/`decrementOrDelete` all hash the token. Existing invite tests set+get by the same plaintext → transparently green; only an at-rest assertion is added.

Consequent test-contract updates (NOT "modifying tests to pass" — the security requirement changes the contract: `getCurrent` can no longer return plaintext): the credential sections of `store.test.ts` (parity) + `pg-invite-credential.test.ts` (bespoke) drop the `getCurrent().toEqual(plaintext-pair)` assertions in favor of `toEqual(pair(sha256Hex(ia), sha256Hex(sc)))` (proves hash-at-rest AND parity — both backends compute identical hashes), and derive the `deleteFor` delete-pair from `getCurrent`'s output (the production pattern). NEW at-rest tests (`at-rest-hashing.test.ts`) assert the raw stored column holds the SHA-256 hash not the plaintext, a valid secret still authenticates, an invalid one rejects, and the plaintext never appears in any stored row (red before impl).

### RF4 design decision (SERIALIZABLE transaction — pg-mem-validated)
Probed pg-mem 3.0.14 (artifact: `scratch-rf4-pgmem-probe.md`): the task's first option — a single `INSERT … SELECT … WHERE (count) AND (byte)` — is **rejected by pg-mem's parser** ("cannot cast type … to list" on the `INSERT…SELECT <projection> WHERE` form, even with explicit casts / `FROM (VALUES …)`); the bare scalar-subquery comparison works in a plain SELECT, so it's a pg-mem `INSERT…SELECT` limitation, not real SQL. The task's second option works: pg-mem accepts `pool.connect()` (client has `query`+`release`), `BEGIN ISOLATION LEVEL SERIALIZABLE`, `FOR UPDATE`, `rollback`, and `bigserial seq` auto-assigns under a txn insert (FIFO preserved). So `enqueue` becomes a **SERIALIZABLE transaction**: `BEGIN ISOLATION LEVEL SERIALIZABLE` → prune → read count+bytes → enforce both bounds (count-first, exact reasons) → conditional INSERT → COMMIT, with a bounded retry on serialization failure (`40001`). SERIALIZABLE (not bare `FOR UPDATE`) is required because the empty-handle race is a phantom — `FOR UPDATE` locks nothing when the handle has no live rows. The `PgPool` structural interface gains `connect()` (real `pg.Pool` + pg-mem both satisfy it). pg-mem is single-threaded so it never raises 40001 — the retry branch is covered by a fake-pool unit test that raises a 40001-coded error once then succeeds.

### Completion Criteria (review fixes)
- [x] RF1: a rejecting `handle()` yields a `500` (not a hang), socket ends, and only a static event name is logged (no message/body/error detail/conn string).
- [x] RF2: emitting `'error'` on the pool is handled (logged static, process does not crash); no conn string / error detail logged.
- [x] RF3: the stored credential columns + invite token column hold the SHA-256 hash, not the plaintext; a valid credential/token still authenticates, an invalid one rejects; the plaintext is unrecoverable from any stored row; restart + parity tests stay green.
- [x] RF4: the bound is enforced exactly + atomically (the atomic enqueue rejects at the cap, count AND bytes); parity with the memory store's behavior + reasons preserved.
- [x] Full gate green: build + typecheck + lint clean, 100%/100%/100%/100% coverage, all tests (incl. dual-backend parity + restart + the interop content-blind headline) pass. *(278 tests, 16 files; 493 stmts / 348 branches / 109 funcs / 463 lines all 100%. Artifact: `final-gate-review-fixes.txt`.)*
- [x] Content-blind assertion still passes against Postgres (the inbox row shape is unchanged by RF4; RF3 touches only credential/invite at-rest form, never the inbox blob). *(The `[postgres] … CONTENT-BLIND` parity test + the interop "relay only ever held CIPHERTEXT" headline both pass; the inbox row is still `{handle, queue_id, message(jsonb), enqueued_at, expires_at, size_bytes, seq}` — no recipient_did, no ciphertext index.)*

### Progress Log (review fixes)
- 2026-06-22 09:40 ALL 4 REVIEW FIXES COMPLETE. Final full gate: build + typecheck + lint clean; 278 tests pass (16 files); 100%/100%/100%/100% coverage (493 stmts / 348 branches / 109 funcs / 463 lines). Content-blind re-confirmed against Postgres (`[postgres] … CONTENT-BLIND` parity test) + the interop "relay only ever held CIPHERTEXT" headline (both green). **Push note:** `git push` to `origin` (github.com/ourostack/friends-relay, a personal/ourostack repo) is rejected — this machine's git HTTPS credential resolves to the `arimendelow_microsoft` EMU account (403, no access), while the correct identity for ourostack is the personal `arimendelow` account (gh is authed as it, but git's credential helper picks the EMU token). All review-fix commits are LOCAL on `feat/persistent-storage`; the operator/merger pushes with the personal credential. No PR opened (per scope).
- 2026-06-22 09:39 RF4 complete (MEDIUM, concurrent-enqueue quota race). `PgInboxStore.enqueue` now runs prune → count+sum (`FOR UPDATE`) → conditional INSERT in ONE `BEGIN ISOLATION LEVEL SERIALIZABLE` transaction (count-first, same `quota_count`/`quota_bytes` reasons), with a bounded retry (5) on `40001`. SERIALIZABLE (not bare FOR UPDATE) because the fresh-handle cap race is a phantom (no rows to lock). `PgPool` gained `connect()` + a structural `PgPoolClient`; real `pg.Pool` + pg-mem both satisfy it. Exact-cap + distinct-reason behavior stays proven by the dual-backend parity suite over pg-mem (81 inbox/parity/restart tests green through the rewrite); pg-mem can't raise a real 40001 (single-threaded), so new `pg-inbox-atomic.test.ts` (4 tests, scripted fake pool) covers the retry-then-succeed (exactly one insert), bounded-retry-propagates-40001, non-40001-propagates-immediately, and throwing-rollback-doesn't-mask paths. Gate: 278 tests pass; 100%/100%/100%/100%; build+typecheck+lint clean. Commit d636e7e.
- 2026-06-22 09:36 RF3 complete (MEDIUM, bearer secrets plaintext at rest). New `src/security/hash.ts` (`sha256Hex`; rationale: high-entropy random secrets ⇒ fast hash, NOT a slow password KDF). TDD: new `at-rest-hashing.test.ts` (5 tests) → RED (Pg rows + memory `getCurrent` held plaintext). Implemented Design B in BOTH backends' credential + invite stores: write/lookup-key boundaries hash the presented plaintext; `getCurrent` returns the stored DIGEST pair (only consumer = rotation revoke → `deleteFor`, which matches as-given, no re-hash — re-hashing would break revoke/rotation). Invite stores hash the token on all three methods (set+get by same plaintext ⇒ transparently green). 11 credential-section assertions in `store.test.ts` (parity, both backends) + `pg-invite-credential.test.ts` updated to the new contract (`getCurrent`→`hashedPair`; `deleteFor` fed from `getCurrent`) — NOT weakening: the security requirement changes the contract (`getCurrent` can no longer return plaintext); both backends compute identical digests ⇒ parity intact. Registry public routing fields (did/agent_card/key_agreement_pubkey) untouched. Gate: 274 tests pass; 100%/100%/100%/100%; typecheck+lint clean. **Restart + dual-backend parity + the interop content-blind headline all stay green.** Commit 57e0b01.
- 2026-06-22 09:32 RF2 complete (HIGH, no `pool.on('error')` → crash). TDD: added a test emitting `'error'` on a `defaultPoolFactory`-built pool → confirmed RED (emit with no listener THROWS = the prod crash; the secret-carrying error surfaced). Fix: `defaultPoolFactory` gains an optional `logger` (default `silentLogger`) and attaches `pool.on('error', () => logger.log('error','pg_pool_error'))` — the error itself (and the connection string) is NEVER passed to the log. Threaded the logger through `PoolFactory` (now `(url, logger?) => PgPool`) + `assemblePostgresStores(url, bounds, poolFactory?, logger?)`; existing 3-arg test calls + `bin.ts` unchanged (logger defaults to silent). Gate: 269 tests pass; 100%/100%/100%/100%; typecheck+lint clean. Commit d1eb1fe.
- 2026-06-22 09:30 RF1 complete (HIGH, http.ts unhandled rejection + hung socket). TDD: added a socket-level test where `handle()` rejects with a secret-carrying error → confirmed RED (5.28s hang + unhandled rejection caught by vitest + leak). Fix: `createServer` gains an optional `logger` (default `silentLogger`); the per-request async IIFE now `.catch()`es → writes a generic `500 {error:internal_error}`, ends the socket, logs the STATIC event `request_failed` with NO fields (the caught error is never inspected → no conn string / body / content leaks). Added a 2nd test (BigInt-unserializable body) to cover the `!res.headersSent` branch honestly (handle resolves → writeHead(200) → stringify throws in res.end → catch with headers already sent → skip the 2nd writeHead, still end). Gate: 268 tests pass; 100%/100%/100%/100%; build+typecheck+lint clean. Commit 4160dd6.
